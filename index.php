...
<head>
    ...
    <title>ProHost — Professional Cloud Workspace</title>
...
<body>
    ...
    <script>
        ...
        function processFiles(files) {
            ...
            // 🔥 NEW: Project-based upload for multiple files
            const projectId = 'project_' + Date.now();
            const projectName = prompt("Enter project name (e.g., MyWebsite):", "MyProject");
            ...
            const uploadId = 'project_' + Date.now();
            createProgressUI(uploadId, projectName + ' (Project)');
            ...
            xhr.addEventListener('load', () => {
                if (xhr.status === 200) {
                    ...
                    if (response.success) {
                        saveToHistory(projectName, response.link, response.projectId);
        ...
        function saveToHistory(fileName, link, projectId = null) {
            ...
            if (isUpdatingMode && updatingHistoryId) {
                ...
                historyData.push({
                    id: projectId || (Date.now().toString() + Math.random().toString(36).substr(2, 5)),
                    fileName: fileName,
                    link: link,
